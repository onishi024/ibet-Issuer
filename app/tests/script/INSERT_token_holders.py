...
def issue_token(token_type, amount):
    """
    トークン発行
    ISSUERが実行
...
def bulk_transfer(token_contract, number):
    """
    一括移転
    :param token_contract: トークンコントラクト
    :param number: 増幅ennううs
    :return: なし
    """
    web3.eth.defaultAccount = ISSUER
    for i in range(number):
        print(i)
        token_contract.functions.transferFrom(ISSUER, web3.eth.accounts[i], 1). \
            transact({'from': ISSUER, 'gas': 4000000})


def main(number, token_type):
    """
    Main処理
    :param number: 増幅件数
    :param token_type: トークン種別
...
if __name__ == "__main__":
    ...
    main(args.number, args.token_type)
